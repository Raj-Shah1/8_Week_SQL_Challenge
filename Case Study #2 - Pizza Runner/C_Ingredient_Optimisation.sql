...
SELECT topping_name AS most_common_extra_topping
FROM extra_rank
...
SELECT topping_name AS most_common_excluded_topping
FROM exclusion_rank
WHERE topping_rank = 1;
...
(
	...
	FROM tmp_customer_orders co
	INNER JOIN tmp_pizza_recipes pr ON co.pizza_id = pr.pizza_id
	INNER JOIN success_order_records so ON so.record_id = co.record_id
	GROUP BY pr.topping_id
)
,topping_exclusion_count AS
(
	SELECT pe.topping_id
		,COUNT(pe.topping_id) AS topping_count
	FROM tmp_pizza_exclusions pe
	INNER JOIN success_order_records so ON so.record_id = pe.record_id
	GROUP BY pe.topping_id
)
