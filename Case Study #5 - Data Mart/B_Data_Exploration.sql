...
SELECT DATE_TRUNC('month', week_date)::DATE AS start_of_month
	,ROUND(SUM(CASE 
				WHEN platform = 'Retail'
					THEN sales
				END) * 100::NUMERIC / SUM(sales)::NUMERIC, 2) AS retail_sales_percent
	,ROUND(SUM(CASE 
				WHEN platform = 'Shopify'
					THEN sales
				END) * 100::NUMERIC / SUM(sales)::NUMERIC, 2) AS shopify_sales_percent
FROM data_mart.clean_weekly_sales
